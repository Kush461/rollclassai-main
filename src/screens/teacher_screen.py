...
from src.database.db import check_teacher_exists,create_teacher,teacher_login,get_teacher_subjects,get_attendance_records
...
def teacher_dashboard(): 
    ...
    with c1: 
        header_dashboard() 
    with c2: 
        st.markdown(f"""<p style="font-family:'Outfit',sans-serif;font-weight:700;font-size:1rem;text-align:right;margin:0;color:#1e293b;">Welcome<br>{teacher_data["name"]}</p>""", unsafe_allow_html=True)
...
def teacher_tab_take_attendance(): 
    ...
    if "added_image_names" not in st.session_state:
        st.session_state.added_image_names = set()
...
def teacher_tab_manage_subjects():
    ...
    if subjects: 
        for subject in subjects: 
            ...
            def share_btn(subj=subject): 
                if st.button(f"Share Code:{subj['name']}",key=f"share_{subj['subject_code']}",icon=":material/share:"): 
                    share_subject_dialog(subj["name"],subj["subject_code"])
                st.space() 
...
def teacher_tab_attendance_records(): 
    ...
    teacher_id = st.session_state.teacher_data["teacher_id"]
    records = get_attendance_records(teacher_id)
    if not records:
        return 
...
def teacher_screen_login(): 
    ...
    st.header("Login with password", divider=False, text_alignment="center") 
    st.space()
    teacher_username = st.text_input("Enter username")
    teacher_pass = st.text_input("Enter password", type="password")
    st.divider()
    with st.container(key="btn_row"):
        btnc1, btnc2 = st.columns(2)
        with btnc1:
            if st.button("Login", icon=":material/passkey:", key="login", shortcut="control+enter", use_container_width=True): 
        ...
        with btnc2:
            if st.button("Register Instead", type="primary", key="register", icon=":material/passkey:", use_container_width=True):
...
def teacher_screen_register(): 
    ...
    with c1: 
        header_dashboard() 
    with c2: 
        if st.button("← Home", type="secondary", key="backtologin", shortcut="control+backspace"): 
            st.session_state["login_type"] = None
            st.rerun()

    st.header("Register your teacher profile", divider=False) 
    st.space()
    st.space() 
    ...
    with st.container(key="btn_row"):
        btnc1, btnc2 = st.columns(2)
        with btnc1:
            if st.button("Login Instead", icon=":material/passkey:", key="login", shortcut="control+enter", use_container_width=True): 
                st.session_state.teacher_login_type="login"
        with btnc2:
            if st.button("Register", type="primary", key="register", icon=":material/passkey:", use_container_width=True): 
